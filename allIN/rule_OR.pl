...
spots(S) :- shopping(S), select(shopping).
spots(S) :- technology(S), select(technology).
spots(S) :- nature(S), select(nature).
spots(S) :- literature_art(S), select(literature_art).
spots(S) :- animal(S), select(animal).
spots(S) :- cultural_history(S), select(cultural_history).
spots(S) :- battlefield(S), select(battlefield).
spots(S) :- taking_picture(S), select(taking_picture).